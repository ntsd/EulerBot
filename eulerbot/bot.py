...
from eulerbot.config import Config, ConfigDefaults
from eulerbot.permissions import Permissions, PermissionsDefaults

from eulerbot import exceptions
from eulerbot.opus_loader import load_opus_lib
from eulerbot.constants import VERSION as BOTVERSION

#euler bot
from . import math_question_generator
...
class EulerBot(discord.Client):
    def __init__(self, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        ...
        self.http.user_agent += ' Euler Bot/%s' % BOTVERSION
    ...
    async def on_ready(self):
        print('\rConnected!  Euler Bot v%s\n' % BOTVERSION)
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += "https://github.com/ntsd/EulerBot"
    ...
    async def cmd_hello(self):
        ...
        print hello bot
        """
        return Response("Hello I'm Euler Bot", delete_after=10)

    math_question = ""
    math_answer = ""
    async def cmd_mathquestion(self, level="Easy"):
        """
        Usage:
            {command_prefix}mathquestion level

        level: Easy, Medium, Hard
        generate math question
        """
        self.math_question, self.math_answer = math_question_generator.question_generator(level)
        lines = 'What is {} \n'.format(self.math_question)
        # lines += "you can use '{}mathanswer answer' to answer".format(self.config.command_prefix)
        return Response(lines)

    async def cmd_mathanswer(self, author, answer):
        """
        Usage:
            {command_prefix}mathanswer answer

        answer the math question
        """
        if self.math_question == "":
            return Response("It's don't has question not use '{}mathquestion level' to generate".format(self.config.command_prefix))
        if self.math_answer == int(answer):
            self.math_question = ""
            print_math_answer = self.math_answer
            self.math_answer = ""
